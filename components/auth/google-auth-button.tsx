  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-alt)] px-4 py-3 text-sm font-bold text-[var(--foreground)] transition hover:bg-[var(--surface-raised)] disabled:opacity-60"
      >
        {isSubmitting ? (
          <span className="h-5 w-5 animate-spin rounded-full border-2 border-[var(--muted)] border-t-[var(--foreground)]" />
        ) : (
          <GoogleIcon />
        )}
