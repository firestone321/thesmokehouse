...
function buildRequestFingerprint(request: Request) {
  ...
  return createHash("sha256").update(`${clientIp}|${userAgent}`).digest("hex").slice(0, 24);
}

function cleanupRepeatedEventStore(now: number) {
  const shouldCleanup =
    lastRepeatedEventCleanupAt === 0 || now - lastRepeatedEventCleanupAt >= REPEATED_EVENT_CLEANUP_INTERVAL_MS;
...
function trackRepeatedSecurityEvent(reportKey: string, thresholds: number[], windowMs: number) {
  const now = Date.now();
...
function writeStructuredLog(severity: SecurityEventSeverity, payload: Record<string, unknown>) {
...
export function logSecurityEvent(input: SecurityEventInput) {
  ...
  const reportKey =
    input.report.key ?? [input.event, requestContext?.path ?? "global", requestContext?.fingerprint ?? "global"].join(":");
